...
# 2. 檢查並啟動 Deno Web 伺服器 (Port 8088，內建阿里 SenseVoice 50x 與 Whisper 雙引擎)
$is8088 = netstat -ano | findstr ":8088" | findstr "LISTENING"
if (-not $is8088) {
    Write-Host "[2/4] 正在啟動會議助理雙引擎伺服器 (SenseVoice 50x + Whisper)..." -ForegroundColor Cyan
...
# 4. 開啟專屬應用程式視窗
Write-Host "🎉 服務與雙引擎已就緒！正在開啟專屬應用程式視窗..." -ForegroundColor Green
